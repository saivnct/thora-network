...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.thora.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.thora.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) Proposals() map[common.Address]bool {
	api.thora.lock.RLock()
	defer api.thora.lock.RUnlock()

	proposals := make(map[common.Address]bool)
	for address, auth := range api.thora.proposals {
...
func (api *API) Propose(address common.Address, auth bool) {
	api.thora.lock.Lock()
	defer api.thora.lock.Unlock()

	api.thora.proposals[address] = auth
}
...
func (api *API) Discard(address common.Address) {
	...
	delete(api.thora.proposals, address)
}
...
// GetSigner returns the signer for a specific thora block.
...
func (api *API) GetSigner(rlpOrBlockNr *blockNumberOrHashOrRLP) (common.Address, error) {
	if len(rlpOrBlockNr.RLP) == 0 {
		...
		return api.thora.Author(header)
	}
	block := new(types.Block)
	if err := rlp.DecodeBytes(rlpOrBlockNr.RLP, block); err == nil {
		return api.thora.Author(block.Header())
	}
