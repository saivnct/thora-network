Name "thora ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install thora binary
Section "Thora" GETH_IDX
  SetOutPath $INSTDIR
  file {{.Geth}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\thora.exe"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\thora.exe" "attach"
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe"

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "Thora incoming peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Thora outgoing peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Thora UDP discovery (UDP:30303)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "Thora incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\thora.exe" "" "" "Thora" 30303 "" "" ""
  SimpleFC::AdvAddRule "Thora outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\thora.exe" "" "" "Thora" "" 30303 "" ""
  SimpleFC::AdvAddRule "Thora UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\thora.exe" "" "" "Thora" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/ethereum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "THORA_SOCKET" "R" "HKLM" "\\.\pipe\thora.ipc"
  ${EnvVarUpdate} $0 "THORA_SOCKET" "A" "HKLM" "\\.\pipe\thora.ipc"
